...
import { LayoutDashboard, Package, Warehouse, ShoppingBag, Users, Tag, BarChart3, Settings, ArrowLeft } from 'lucide-react';
...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <aside className="w-full lg:w-64 glass-panel rounded-3xl p-4 sm:p-5 flex flex-col justify-between shrink-0 border border-white/80">
      ...
      {/* Return to Customer App Link */}
      <div className="pt-4 border-t border-slate-200/80 mt-6">
        <Link
          to="/"
          className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-2xl text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors"
        >
